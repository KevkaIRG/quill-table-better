    ...
    const DEFAULT: MenusDefaults = {
        ...
        table: {
            ...
            handler(list: HTMLUListElement, tooltip: HTMLDivElement) {
                ...
                this.toggleAttribute(list, tooltip);
                this.tablePropertiesForm = new TablePropertiesForm(this, {
                    attribute,
                    type: 'table'
                }, this.getColorList('table'), this.getUseDimension('table'));
                this.hideMenus();
            }
        },
        cell: {
            ...
            handler(list: HTMLUListElement, tooltip: HTMLDivElement) {
                ...
                this.tablePropertiesForm = new TablePropertiesForm(this, {
                    attribute,
                    type: 'cell'
                }, this.getColorList('cell'), this.getUseDimension('cell'));
                this.hideMenus();
            }
        },
        wrap: {
...
class TableMenus {
    ...
    getCorrectLeft(containerBounds: CorrectBound, boundingBox: BoundingBoxRect, width: number) {
    ...
    getCorrectTop(boundingBox: BoundingBoxRect, computedStyle: CSSStyleDeclaration, height: number) {
        ...
        if (!isTooltipBottom) {
            ...
            correctTop = boundingBox.bottom + 10;
    ...
    getBoundingBoxMeridian() {
        ...
        if (!this.tableBetter || !this.tableBetter.cellSelection || !this.tableBetter.cellSelection.selectedTds || this.tableBetter.cellSelection.selectedTds.length === 0) {
            return boundingBox;
        }
        const selectedTds = this.tableBetter.cellSelection.selectedTds;
        boundingBox.isBoundingRect = true;
        if (selectedTds.length === 1) {
            const selectionBounds = getCorrectBounds(selectedTds[0], this.quill.container);
            ...
            selectedTds.forEach((td) => {
                const rect = getCorrectBounds(td, this.quill.container);
